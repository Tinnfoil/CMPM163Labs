...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        public Vector2 Inputt       { get; private set; }
		...
		{
			float height = 0;
			if (Input.GetKeyDown(KeyCode.Space) && CountGroundedWheels(out height) > 1)
			{
				Jumped = true;	
			}
		}
        ...
        void MoveVehicle(float accelInput, float turnInput)
        {
			...
			//Add Sparks depending on speed
			var eml = SparksLeft.emission;
			eml.rateOverTime = (int)adjustedVelocity.magnitude;
			var emr = SparksRight.emission;
			emr.rateOverTime = (int)adjustedVelocity.magnitude;
			if (Jumped)
			{
				adjustedVelocity += Vector3.up * 6;
				Jumped = false;
			}
        ...
        bool IsStuck()
        {
            ...
            if(GroundPercent <= 0 && speed < 0.01f && Mathf.Abs(Inputt.y) > 0)
                return true;
